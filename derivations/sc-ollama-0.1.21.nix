{ lib, buildGoModule, fetchFromGitHub, makeWrapper, cudaSupport ? false
, llama-cpp, config ? { } }:

let llama-cppNonStatic = (llama-cpp.override { static = false; });
in buildGoModule rec {
  ...
  nativeBuildInputs = lib.optionals cudaSupport [ makeWrapper ];
